		...
		if (!IsCursorActive() && IsWorldInitialized()) {
			...
			if (isW) {
				if (Character && Character->_Camera) {
					...
					Character->moveForward(3.0f * (WorldEngine::VulkanDriver::deltaFrame));
				}
				else {
			...
			if (isS) {
				if (Character && Character->_Camera) {
					Character->moveBackward(3.0f * (WorldEngine::VulkanDriver::deltaFrame));
				}
				else {
			...
			if (isA) {
				if (Character && Character->_Camera) {
					Character->moveLeft(3.0f * (WorldEngine::VulkanDriver::deltaFrame));
				}
				else {
			...
			if (isD) {
				if (Character && Character->_Camera) {
					...
					Character->moveRight(3.0f * (WorldEngine::VulkanDriver::deltaFrame));
				}
				else {
