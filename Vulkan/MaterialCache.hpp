...
public:
	...
	PipelineObject* GetPipe(Pipelines PipeType) {
	...
	TextureObject* createTextureImage(const char* File) {
		...
		TextureObject* Tex = new TextureObject(_Driver,texWidth,texHeight,pixels);
...
//		CPU->GPU Copy
		...
		if (vkCreateDescriptorPool(_Driver->device, &poolInfo, nullptr, &Tex->DescriptorPool) != VK_SUCCESS) {
#ifdef _DEBUG
			throw std::runtime_error("failed to create descriptor pool!");
#endif
		}

		auto Pipe = GetPipe(Pipelines::GUI);
		...
		allocInfoDS.descriptorPool = Tex->DescriptorPool;
		allocInfoDS.descriptorSetCount = static_cast<uint32_t>(_Driver->swapChainImages.size());
		allocInfoDS.pSetLayouts = layouts.data();

		Tex->DescriptorSets.resize(_Driver->swapChainImages.size());
		if (vkAllocateDescriptorSets(_Driver->device, &allocInfoDS, Tex->DescriptorSets.data()) != VK_SUCCESS) {
#ifdef _DEBUG
		...
		for (size_t i = 0; i < _Driver->swapChainImages.size(); i++) {
			...
			descriptorWrites[0].dstSet = Tex->DescriptorSets[i];
	...
	void CreateDefault() {
		...
		Pipes.emplace_back(new PipelineObject(_Driver));
		PipelineObject* Pipe = Pipes.back();
		...
		Pipe->createDescriptorSetLayout(layoutInfo);
		...
		Pipe->createPipeline(shaderStages, vertexInputInfo, inputAssembly, viewportState, rasterizer, multisampling, depthStencil, colorBlending, dynamicState);
	...
	void CreateGUI() {
		...
		//	Graphics Pipeline
		//
		auto vertShaderCode = readFile("Vertex2.vert.spv");
		auto fragShaderCode = readFile("Fragment2.frag.spv");
		VkShaderModule vertShaderModule = createShaderModule(vertShaderCode);
		VkShaderModule fragShaderModule = createShaderModule(fragShaderCode);
