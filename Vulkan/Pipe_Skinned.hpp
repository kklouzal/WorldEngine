...
namespace Pipeline {
	struct Skinned : public PipelineObject {

		VulkanDriver* _Driver;

		Skinned(VulkanDriver* Driver) : PipelineObject(Driver), _Driver(Driver) {
			//
			//	DescriptorSetLayout
			//
			...
			//	Graphics Pipeline
			//
			auto vertShaderCode = readFile("Vertex_Skinned.vert.spv");
			auto fragShaderCode = readFile("Fragment_Skinned.frag.spv");
