...
public:
	...
	*/
	Framebuffer(VulkanDevice* vulkanDevice, VmaAllocator vmaAllocator)
	{
		assert(vulkanDevice);
	...
	*/
	~Framebuffer()
	{
		...
		{
			vmaDestroyImage(vmaAllocator, attachment.image, attachment.imageAlloc);
	...
	*/
	uint32_t addAttachment(AttachmentCreateInfo createinfo)
	{
		...
		vmaCreateImage(vmaAllocator, &image, &allocInfo, &attachment.image, &attachment.imageAlloc, nullptr);
