...
DecompResults* Decomp(GLTFInfo* Infos) {
	...
	for (uint32_t i = 0; i < nPoints; i++) {
		Points.push_back(Infos->Vertices[Infos->Indices[i]].pos.x);
		Points.push_back(Infos->Vertices[Infos->Indices[i]].pos.y);
		Points.push_back(Infos->Vertices[Infos->Indices[i]].pos.z);
	}
	//
